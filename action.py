...
class BulbOnOff():
	...
	def run(self, command):
		...
		if bulb.state==requestedState:
			print("Nothing to do")
		else:
			bulb.state=requestedState
...
class BedBulbOnOff():
	...
	def run(self, command):
		env = Environment('Mini')
		env.start()
		env.discover(seconds=5)
		switch = env.get_switch('Mini')
		if command=='bedroom light on':
			switch.on()
...
class play(object):
	...
	def run(self, command):
		artist = re.sub('play','',command,flags=re.IGNORECASE)
		artist=artist.lstrip()
		print(artist)
		api = Mobileclient()
		logged_in = api.login('username', 'password',Mobileclient.FROM_MAC_ADDRESS)
		lib=api.get_all_songs()
		track_ids=[track['id'] for track in lib if track['artist'].lower() ==artist.lower()]
		print(track_ids)
		if len(track_ids)>0:
			song=api.get_stream_url(track_ids[0])
			print (song)
			call(['mpv','--really-quiet', song])
...
def make_actor(say):
    ...
    # =========================================
    actor.add_keyword('lamp on', BulbOnOff(say))
    actor.add_keyword('lamp off', BulbOnOff(say))
    actor.add_keyword('bedroom light off', BedBulbOnOff(say))
    actor.add_keyword(_('bedroom light on'), BedBulbOnOff(say))
